...
import {
  FACILITY_CATEGORIES,
  AIZUMI_EXPECTED_CATEGORY_COUNTS,
  AIZUMI_EXPECTED_GEO_COUNT,
  AIZUMI_EXPECTED_HOURS_COUNT,
  AIZUMI_EXPECTED_MISSING_ADDRESS,
  AIZUMI_EXPECTED_MISSING_PHONE,
  AIZUMI_EXPECTED_ROW_COUNT,
  AIZUMI_PACK_ACCESSED,
  AIZUMI_PACK_JIS,
  type FacilityCategory,
  type FacilityRow
} from './facility-schema';

const PACK_PATH = path.join(process.cwd(), 'data', 'frozen', 'aizumi-facilities.jsonl');
...
function parseJis(value: unknown): string {
  if (value !== AIZUMI_PACK_JIS) {
    throw new Error(`facility jis must be ${AIZUMI_PACK_JIS}`);
  }
  return AIZUMI_PACK_JIS;
}
...
function parseFacilityRow(raw: unknown): FacilityRow {
  ...
  if (accessed !== AIZUMI_PACK_ACCESSED) {
    throw new Error(`facility accessed must be ${AIZUMI_PACK_ACCESSED}`);
  }
  return {
...
function loadNakaFacilities(): readonly FacilityRow[] {
  ...
  if (rows.length !== AIZUMI_EXPECTED_ROW_COUNT) {
    throw new Error(`naka pack row count ${rows.length} != ${AIZUMI_EXPECTED_ROW_COUNT}`);
  }
  ...
  for (const cat of FACILITY_CATEGORIES) {
    if (counts[cat] !== AIZUMI_EXPECTED_CATEGORY_COUNTS[cat]) {
      throw new Error(
        `naka pack ${cat} ${counts[cat]} != ${AIZUMI_EXPECTED_CATEGORY_COUNTS[cat]}`
      );
    }
  }
  let geo = 0;
  ...
  if (geo !== AIZUMI_EXPECTED_GEO_COUNT) {
    throw new Error(`naka pack geo ${geo} != ${AIZUMI_EXPECTED_GEO_COUNT}`);
  }
  if (hours !== AIZUMI_EXPECTED_HOURS_COUNT) {
    throw new Error(`naka pack hours ${hours} != ${AIZUMI_EXPECTED_HOURS_COUNT}`);
  }
  if (missingAddress !== AIZUMI_EXPECTED_MISSING_ADDRESS) {
    throw new Error(
      `naka pack address gaps ${missingAddress} != ${AIZUMI_EXPECTED_MISSING_ADDRESS}`
    );
  }
  if (missingPhone !== AIZUMI_EXPECTED_MISSING_PHONE) {
    throw new Error(
      `naka pack phone gaps ${missingPhone} != ${AIZUMI_EXPECTED_MISSING_PHONE}`
    );
  }
  return rows;
}

export const AIZUMI_FACILITIES: readonly FacilityRow[] = loadNakaFacilities();
