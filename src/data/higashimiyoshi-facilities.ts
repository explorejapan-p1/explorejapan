...
function parseJis(value: unknown): string {
  if (value !== HIGASHIMIYOSHI_PACK_JIS) {
    throw new Error(`facility jis must be ${HIGASHIMIYOSHI_PACK_JIS}`);
  }
  return HIGASHIMIYOSHI_PACK_JIS;
}
...
function loadHigashimiyoshiFacilities(): readonly FacilityRow[] {
