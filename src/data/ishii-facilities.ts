...
import {
  FACILITY_CATEGORIES,
  ISHII_EXPECTED_CATEGORY_COUNTS,
  ISHII_EXPECTED_GEO_COUNT,
  ISHII_EXPECTED_HOURS_COUNT,
  ISHII_EXPECTED_MISSING_ADDRESS,
  ISHII_EXPECTED_MISSING_PHONE,
...
const PACK_PATH = path.join(process.cwd(), 'data', 'frozen', 'ishii-facilities.jsonl');
...
function parseJis(value: unknown): string {
  if (value !== ISHII_PACK_JIS) {
    throw new Error(`facility jis must be ${ISHII_PACK_JIS}`);
  }
  return ISHII_PACK_JIS;
}
...
function parseFacilityRow(raw: unknown): FacilityRow {
  ...
  if (accessed !== ISHII_PACK_ACCESSED) {
    throw new Error(`facility accessed must be ${ISHII_PACK_ACCESSED}`);
  }
  return {
...
function loadMatsushigeFacilities(): readonly FacilityRow[] {
  ...
  if (rows.length !== ISHII_EXPECTED_ROW_COUNT) {
    throw new Error(`ishii pack row count ${rows.length} != ${ISHII_EXPECTED_ROW_COUNT}`);
  }
  ...
  for (const cat of FACILITY_CATEGORIES) {
    if (counts[cat] !== ISHII_EXPECTED_CATEGORY_COUNTS[cat]) {
      throw new Error(
        `ishii pack ${cat} ${counts[cat]} != ${ISHII_EXPECTED_CATEGORY_COUNTS[cat]}`
      );
    }
  }
  let geo = 0;
  ...
  if (geo !== ISHII_EXPECTED_GEO_COUNT) {
    throw new Error(`ishii pack geo ${geo} != ${ISHII_EXPECTED_GEO_COUNT}`);
  }
  if (hours !== ISHII_EXPECTED_HOURS_COUNT) {
    throw new Error(`ishii pack hours ${hours} != ${ISHII_EXPECTED_HOURS_COUNT}`);
  }
  if (missingAddress !== ISHII_EXPECTED_MISSING_ADDRESS) {
    throw new Error(
      `ishii pack address gaps ${missingAddress} != ${ISHII_EXPECTED_MISSING_ADDRESS}`
    );
  }
  if (missingPhone !== ISHII_EXPECTED_MISSING_PHONE) {
    throw new Error(
      `ishii pack phone gaps ${missingPhone} != ${ISHII_EXPECTED_MISSING_PHONE}`
    );
  }
  return rows;
}

export const ISHII_FACILITIES: readonly FacilityRow[] = loadMatsushigeFacilities();
