...
function parseJis(value: unknown): string {
  if (value !== KAMIKATSU_PACK_JIS) {
    throw new Error(`facility jis must be ${KAMIKATSU_PACK_JIS}`);
  }
  return KAMIKATSU_PACK_JIS;
}
...
function parseFacilityRow(raw: unknown): FacilityRow {
  ...
  if (accessed !== KAMIKATSU_PACK_ACCESSED) {
    throw new Error(`facility accessed must be ${KAMIKATSU_PACK_ACCESSED}`);
  }
  return {
...
function loadKamikatsuFacilities(): readonly FacilityRow[] {
