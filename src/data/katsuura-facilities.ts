...
import {
  FACILITY_CATEGORIES,
  KATSUURA_EXPECTED_CATEGORY_COUNTS,
  KATSUURA_EXPECTED_GEO_COUNT,
  KATSUURA_EXPECTED_HOURS_COUNT,
  KATSUURA_EXPECTED_MISSING_ADDRESS,
  KATSUURA_EXPECTED_MISSING_PHONE,
  KATSUURA_EXPECTED_ROW_COUNT,
  KATSUURA_PACK_ACCESSED,
  KATSUURA_PACK_JIS,
  type FacilityCategory,
  type FacilityRow
} from './facility-schema';

const PACK_PATH = path.join(process.cwd(), 'data', 'frozen', 'katsuura-facilities.jsonl');
...
function parseJis(value: unknown): string {
  if (value !== KATSUURA_PACK_JIS) {
    throw new Error(`facility jis must be ${KATSUURA_PACK_JIS}`);
  }
  return KATSUURA_PACK_JIS;
}
...
function parseFacilityRow(raw: unknown): FacilityRow {
  ...
  if (accessed !== KATSUURA_PACK_ACCESSED) {
    throw new Error(`facility accessed must be ${KATSUURA_PACK_ACCESSED}`);
  }
  return {
...
function loadKatsuuraFacilities(): readonly FacilityRow[] {
  ...
  if (rows.length !== KATSUURA_EXPECTED_ROW_COUNT) {
    throw new Error(`katsuura pack row count ${rows.length} != ${KATSUURA_EXPECTED_ROW_COUNT}`);
  }
  ...
  for (const cat of FACILITY_CATEGORIES) {
    if (counts[cat] !== KATSUURA_EXPECTED_CATEGORY_COUNTS[cat]) {
      throw new Error(
        `katsuura pack ${cat} ${counts[cat]} != ${KATSUURA_EXPECTED_CATEGORY_COUNTS[cat]}`
      );
    }
  }
  let geo = 0;
  ...
  if (geo !== KATSUURA_EXPECTED_GEO_COUNT) {
    throw new Error(`katsuura pack geo ${geo} != ${KATSUURA_EXPECTED_GEO_COUNT}`);
  }
  if (hours !== KATSUURA_EXPECTED_HOURS_COUNT) {
    throw new Error(`katsuura pack hours ${hours} != ${KATSUURA_EXPECTED_HOURS_COUNT}`);
  }
  if (missingAddress !== KATSUURA_EXPECTED_MISSING_ADDRESS) {
    throw new Error(
      `katsuura pack address gaps ${missingAddress} != ${KATSUURA_EXPECTED_MISSING_ADDRESS}`
    );
  }
  if (missingPhone !== KATSUURA_EXPECTED_MISSING_PHONE) {
    throw new Error(
      `katsuura pack phone gaps ${missingPhone} != ${KATSUURA_EXPECTED_MISSING_PHONE}`
    );
  }
  return rows;
}

export const KATSUURA_FACILITIES: readonly FacilityRow[] = loadKatsuuraFacilities();
