...
import {
  FACILITY_CATEGORIES,
  MATSUSHIGE_EXPECTED_CATEGORY_COUNTS,
  MATSUSHIGE_EXPECTED_GEO_COUNT,
  MATSUSHIGE_EXPECTED_HOURS_COUNT,
  MATSUSHIGE_EXPECTED_MISSING_ADDRESS,
  MATSUSHIGE_EXPECTED_MISSING_PHONE,
...
function loadMatsushigeFacilities(): readonly FacilityRow[] {
  ...
  if (rows.length !== MATSUSHIGE_EXPECTED_ROW_COUNT) {
    throw new Error(`matsushige pack row count ${rows.length} != ${MATSUSHIGE_EXPECTED_ROW_COUNT}`);
  }
  ...
  for (const cat of FACILITY_CATEGORIES) {
    if (counts[cat] !== MATSUSHIGE_EXPECTED_CATEGORY_COUNTS[cat]) {
      throw new Error(
        `matsushige pack ${cat} ${counts[cat]} != ${MATSUSHIGE_EXPECTED_CATEGORY_COUNTS[cat]}`
      );
    }
  }
  let geo = 0;
  ...
  if (geo !== MATSUSHIGE_EXPECTED_GEO_COUNT) {
    throw new Error(`matsushige pack geo ${geo} != ${MATSUSHIGE_EXPECTED_GEO_COUNT}`);
  }
  if (hours !== MATSUSHIGE_EXPECTED_HOURS_COUNT) {
    throw new Error(`matsushige pack hours ${hours} != ${MATSUSHIGE_EXPECTED_HOURS_COUNT}`);
  }
  if (missingAddress !== MATSUSHIGE_EXPECTED_MISSING_ADDRESS) {
    throw new Error(
      `matsushige pack address gaps ${missingAddress} != ${MATSUSHIGE_EXPECTED_MISSING_ADDRESS}`
    );
  }
  if (missingPhone !== MATSUSHIGE_EXPECTED_MISSING_PHONE) {
    throw new Error(
      `matsushige pack phone gaps ${missingPhone} != ${MATSUSHIGE_EXPECTED_MISSING_PHONE}`
    );
  }
  return rows;
}

export const MATSUSHIGE_FACILITIES: readonly FacilityRow[] = loadMatsushigeFacilities();
