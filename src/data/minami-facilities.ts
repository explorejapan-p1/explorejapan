...
import {
  FACILITY_CATEGORIES,
  MINAMI_EXPECTED_CATEGORY_COUNTS,
  MINAMI_EXPECTED_GEO_COUNT,
  MINAMI_EXPECTED_HOURS_COUNT,
  MINAMI_EXPECTED_MISSING_ADDRESS,
  MINAMI_EXPECTED_MISSING_PHONE,
...
function loadMinamiFacilities(): readonly FacilityRow[] {
  ...
  if (rows.length !== MINAMI_EXPECTED_ROW_COUNT) {
    throw new Error(`minami pack row count ${rows.length} != ${MINAMI_EXPECTED_ROW_COUNT}`);
  }
  ...
  for (const cat of FACILITY_CATEGORIES) {
    if (counts[cat] !== MINAMI_EXPECTED_CATEGORY_COUNTS[cat]) {
      throw new Error(
        `minami pack ${cat} ${counts[cat]} != ${MINAMI_EXPECTED_CATEGORY_COUNTS[cat]}`
      );
    }
  }
  let geo = 0;
  ...
  if (geo !== MINAMI_EXPECTED_GEO_COUNT) {
    throw new Error(`minami pack geo ${geo} != ${MINAMI_EXPECTED_GEO_COUNT}`);
  }
  if (hours !== MINAMI_EXPECTED_HOURS_COUNT) {
    throw new Error(`minami pack hours ${hours} != ${MINAMI_EXPECTED_HOURS_COUNT}`);
  }
  if (missingAddress !== MINAMI_EXPECTED_MISSING_ADDRESS) {
    throw new Error(
      `minami pack address gaps ${missingAddress} != ${MINAMI_EXPECTED_MISSING_ADDRESS}`
    );
  }
  if (missingPhone !== MINAMI_EXPECTED_MISSING_PHONE) {
    throw new Error(
      `minami pack phone gaps ${missingPhone} != ${MINAMI_EXPECTED_MISSING_PHONE}`
    );
  }
  return rows;
}

export const MINAMI_FACILITIES: readonly FacilityRow[] = loadMinamiFacilities();
