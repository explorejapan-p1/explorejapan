...
import {
  ...
  HIROSHIMA_MUNICIPALITY_BY_SLUG
} from './hiroshima-municipalities';
import {
  OKAYAMA_MUNICIPALITIES,
  OKAYAMA_MUNICIPALITY_BY_SLUG
} from './okayama-municipalities';
import {
...
export function municipalitiesForPref(pref: string): Municipality[] {
  ...
  if (pref === 'hiroshima') return HIROSHIMA_MUNICIPALITIES;
  if (pref === 'okayama') return OKAYAMA_MUNICIPALITIES;
  return [];
}
...
export function municipalityBySlug(
  ...
  if (pref === 'hiroshima') return HIROSHIMA_MUNICIPALITY_BY_SLUG.get(slug);
  if (pref === 'okayama') return OKAYAMA_MUNICIPALITY_BY_SLUG.get(slug);
  return undefined;
}

export function prefHasMunicipalityLayer(pref: string): pref is PrefSlug {
  return pref === 'tokushima' || pref === 'kagawa' || pref === 'kochi' || pref === 'ehime' || pref === 'hiroshima' || pref === 'okayama';
}
...
export function allMunicipalityStaticParams(): {prefecture: string; municipality: string}[] {
  return [
    ...
    ...HIROSHIMA_MUNICIPALITIES.map((m) => ({prefecture: 'hiroshima', municipality: m.slug})),
    ...OKAYAMA_MUNICIPALITIES.map((m) => ({prefecture: 'okayama', municipality: m.slug}))
  ];
}
