...
import {
  FACILITY_CATEGORIES,
  SANAGOCHI_EXPECTED_CATEGORY_COUNTS,
  SANAGOCHI_EXPECTED_GEO_COUNT,
  SANAGOCHI_EXPECTED_HOURS_COUNT,
  SANAGOCHI_EXPECTED_MISSING_ADDRESS,
  SANAGOCHI_EXPECTED_MISSING_PHONE,
...
function loadKatsuuraFacilities(): readonly FacilityRow[] {
  ...
  if (rows.length !== SANAGOCHI_EXPECTED_ROW_COUNT) {
    throw new Error(`sanagochi pack row count ${rows.length} != ${SANAGOCHI_EXPECTED_ROW_COUNT}`);
  }
  ...
  for (const cat of FACILITY_CATEGORIES) {
    if (counts[cat] !== SANAGOCHI_EXPECTED_CATEGORY_COUNTS[cat]) {
      throw new Error(
        `sanagochi pack ${cat} ${counts[cat]} != ${SANAGOCHI_EXPECTED_CATEGORY_COUNTS[cat]}`
      );
    }
  }
  let geo = 0;
  ...
  if (geo !== SANAGOCHI_EXPECTED_GEO_COUNT) {
    throw new Error(`sanagochi pack geo ${geo} != ${SANAGOCHI_EXPECTED_GEO_COUNT}`);
  }
  if (hours !== SANAGOCHI_EXPECTED_HOURS_COUNT) {
    throw new Error(`sanagochi pack hours ${hours} != ${SANAGOCHI_EXPECTED_HOURS_COUNT}`);
  }
  if (missingAddress !== SANAGOCHI_EXPECTED_MISSING_ADDRESS) {
    throw new Error(
      `sanagochi pack address gaps ${missingAddress} != ${SANAGOCHI_EXPECTED_MISSING_ADDRESS}`
    );
  }
  if (missingPhone !== SANAGOCHI_EXPECTED_MISSING_PHONE) {
    throw new Error(
      `sanagochi pack phone gaps ${missingPhone} != ${SANAGOCHI_EXPECTED_MISSING_PHONE}`
    );
  }
  return rows;
}

export const SANAGOCHI_FACILITIES: readonly FacilityRow[] = loadKatsuuraFacilities();
