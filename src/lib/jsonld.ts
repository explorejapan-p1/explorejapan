...
export function kaitaGraph(locale: AppLocale) {
  const url = canonicalUrl(locale, 'hiroshima/kaita');
  const origin = siteOrigin();
  const isJa = locale === 'ja';
  const featured = featuredListings('kaita');
  return {
    '@context': 'https://schema.org',
    '@graph': [
      {
        '@type': 'TouristDestination',
        '@id': `${url}#place`,
        name: isJa ? KAITA.nameJa : KAITA.nameEn,
        alternateName: isJa ? KAITA.nameEn : KAITA.nameJa,
        identifier: KAITA.jis,
        url,
        image: photoAbs(KAITA_PLACE_PHOTO),
        sameAs: [KAITA.sameAs],
        address: {
          '@type': 'PostalAddress',
          streetAddress: isJa ? '南昭和町14番17号' : '14-17 Minami-Showacho',
          addressLocality: isJa ? KAITA.nameJa : KAITA.nameEn,
          addressRegion: isJa ? KAITA.prefectureJa : KAITA.prefectureEn,
          postalCode: KAITA.hall.postalCode,
          addressCountry: 'JP'
        },
        containedInPlace: {
          '@type': 'AdministrativeArea',
          name: isJa ? KAITA.prefectureJa : KAITA.prefectureEn
        }
      },
      {
        '@type': 'WebPage',
...
export function ozuGraph(locale: AppLocale) {
  ...
  const origin = siteOrigin();
  const isJa = locale === 'ja';
  const featured = featuredListings('ozu');
  return {
    '@context': 'https://schema.org',
    '@graph': [
      {
        '@type': ['AdministrativeArea', 'TouristDestination'],
        '@id': `${url}#place`,
        name: isJa ? OZU.nameJa : OZU.nameEn,
        alternateName: isJa ? OZU.nameEn : OZU.nameJa,
        identifier: OZU.jis,
        url,
        image: photoAbs(OZU_PLACE_PHOTO),
        sameAs: [OZU.sameAs],
        address: {
          '@type': 'PostalAddress',
          streetAddress: isJa ? '大洲690番地の1' : '690-1 Ozu',
          addressLocality: isJa ? OZU.nameJa : OZU.nameEn,
          addressRegion: isJa ? OZU.prefectureJa : OZU.prefectureEn,
          postalCode: OZU.hall.postalCode,
          addressCountry: 'JP'
        },
        containedInPlace: {
          '@type': 'AdministrativeArea',
          name: isJa ? OZU.prefectureJa : OZU.prefectureEn
        }
      },
      {
        '@type': 'WebPage',
      ...
      },
      {
        ...
        itemListElement: [
          {'@type': 'ListItem', position: 1, name: isJa ? '全国' : 'Japan', item: canonicalUrl(locale)},
          {'@type': 'ListItem', position: 2, name: isJa ? OZU.prefectureJa : OZU.prefectureEn, item: canonicalUrl(locale, 'ehime')},
          {'@type': 'ListItem', position: 3, name: isJa ? OZU.nameJa : OZU.nameEn, item: url}
        ]
      },
      {
        '@type': 'ItemList',
        name: isJa ? '大洲市の案内' : 'Places in Ozu City',
        numberOfItems: featured.length,
        itemListElement: featured.map((row, index) => ({
          '@type': 'ListItem',
          position: index + 1,
