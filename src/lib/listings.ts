...
} from '@/data/higashihiroshima-travel';
import {
  HATSUKAICHI_DINING_NAME_SET,
...
import {
  KITAHIROSHIMA_DINING_NAME_SET,
  isKitahiroshimaOnsenPackRow,
  isKitahiroshimaExperiencePackRow,
  isKitahiroshimaStayPackRow,
  rankKitahiroshimaSeeRows,
  kitahiroshimaSightPhoto
} from '@/data/kitahiroshima-travel';
import {
  OSAKIKAMIJIMA_DINING_NAME_SET,
...
} from '@/data/osakikamijima-travel';
import {
  isSeraOnsenPackRow,
  isSeraExperiencePackRow,
  isSeraStayPackRow,
  SERA_DINING_NAME_SET,
  rankSeraSeeRows,
  seraSightPhoto
} from '@/data/sera-travel';
import {
  JINSEIKOGEN_DINING_NAME_SET,
  isJinseikogenOnsenPackRow,
  isJinseikogenExperiencePackRow,
  isJinseikogenStayPackRow,
...
import {
  KURASHIKI_DINING_NAME_SET,
  isKurashikiOnsenPackRow,
  isKurashikiExperiencePackRow,
  isKurashikiStayPackRow,
  rankKurashikiSeeRows,
  kurashikiSightPhoto
} from '@/data/kurashiki-travel';
import {
  TSUYAMA_DINING_NAME_SET,
  isTsuyamaOnsenPackRow,
  isTsuyamaExperiencePackRow,
  isTsuyamaStayPackRow,
  rankTsuyamaSeeRows,
  tsuyamaSightPhoto
} from '@/data/tsuyama-travel';
...
function miyoshishiListings(): PublicListing[] {
  const town = lookupTown('miyoshishi')!;
  const out: PublicListing[] = town.travelAll.map((row) =>
    fromTravel(row, 'miyoshishi', miyoshishiSightPhoto(row.name_ja))
  );
  const seen = new Set<string>();
  const pack: FacilityRow[] = [];
  for (const row of town.rows) {
    if (MIYOSHISHI_DINING_NAME_SET.has(row.name_ja)) continue;
    if (
      !isMiyoshishiOnsenPackRow(row) &&
      !isMiyoshishiExperiencePackRow(row) &&
      !isMiyoshishiStayPackRow(row) &&
      !isSightsCategory(row.category)
    ) {
      continue;
    }
  ...
  const ranked = rankMiyoshishiSeeRows(pack);
  const onsen = pack.filter(isMiyoshishiOnsenPackRow);
  const experience = pack.filter(isMiyoshishiExperiencePackRow);
  const stay = pack.filter(isMiyoshishiStayPackRow);
  for (const row of [...stay, ...onsen, ...experience, ...ranked]) {
    const kind: ListingKind = isMiyoshishiOnsenPackRow(row)
      ? 'onsen'
      : isMiyoshishiExperiencePackRow(row)
        ? 'experience'
        : isMiyoshishiStayPackRow(row)
          ? 'stay'
          : 'sights';
    out.push(fromPack(row, 'miyoshishi', kind, miyoshishiSightPhoto(row.name_ja)));
  }
  return out;
}
...
function fuchuchoListings(): PublicListing[] {
  const town = lookupTown('fuchucho')!;
  const out: PublicListing[] = town.travelAll.map((row) =>
    fromTravel(row, 'fuchucho', fuchuchoSightPhoto(row.name_ja))
  );
  const seen = new Set<string>();
  const pack: FacilityRow[] = [];
  for (const row of town.rows) {
    if (FUCHUCHO_DINING_NAME_SET.has(row.name_ja)) continue;
    if (
      !isFuchuchoOnsenPackRow(row) &&
      !isFuchuchoExperiencePackRow(row) &&
      !isFuchuchoStayPackRow(row) &&
      !isSightsCategory(row.category)
    ) {
      continue;
    }
  ...
  const ranked = rankFuchuchoSeeRows(pack);
  const onsen = pack.filter(isFuchuchoOnsenPackRow);
  const experience = pack.filter(isFuchuchoExperiencePackRow);
  const stay = pack.filter(isFuchuchoStayPackRow);
  for (const row of [...stay, ...onsen, ...experience, ...ranked]) {
    const kind: ListingKind = isFuchuchoOnsenPackRow(row)
      ? 'onsen'
      : isFuchuchoExperiencePackRow(row)
        ? 'experience'
        : isFuchuchoStayPackRow(row)
          ? 'stay'
          : 'sights';
    out.push(fromPack(row, 'fuchucho', kind, fuchuchoSightPhoto(row.name_ja)));
  }
  return out;
}
function kaitaListings(): PublicListing[] {
  const town = lookupTown('kaita')!;
  const out: PublicListing[] = town.travelAll.map((row) =>
    fromTravel(row, 'kaita', kaitaSightPhoto(row.name_ja))
  );
  const seen = new Set<string>();
  const pack: FacilityRow[] = [];
  for (const row of town.rows) {
    if (KAITA_DINING_NAME_SET.has(row.name_ja)) continue;
    if (
      !isKaitaOnsenPackRow(row) &&
      !isKaitaExperiencePackRow(row) &&
      !isKaitaStayPackRow(row) &&
      !isSightsCategory(row.category)
    ) {
      continue;
    }
  ...
  const ranked = rankKaitaSeeRows(pack);
  const onsen = pack.filter(isKaitaOnsenPackRow);
  const experience = pack.filter(isKaitaExperiencePackRow);
  const stay = pack.filter(isKaitaStayPackRow);
  for (const row of [...stay, ...onsen, ...experience, ...ranked]) {
    const kind: ListingKind = isKaitaOnsenPackRow(row)
      ? 'onsen'
      : isKaitaExperiencePackRow(row)
        ? 'experience'
        : isKaitaStayPackRow(row)
          ? 'stay'
          : 'sights';
    out.push(fromPack(row, 'kaita', kind, kaitaSightPhoto(row.name_ja)));
  }
  return out;
}
...
function kurashikiListings(): PublicListing[] {
  const town = lookupTown('kurashiki')!;
  const out: PublicListing[] = town.travelAll.map((row) =>
    fromTravel(row, 'kurashiki', kurashikiSightPhoto(row.name_ja))
  );
  const seen = new Set<string>();
  const pack: FacilityRow[] = [];
  for (const row of town.rows) {
    if (KURASHIKI_DINING_NAME_SET.has(row.name_ja)) continue;
    if (
      !isKurashikiOnsenPackRow(row) &&
      !isKurashikiExperiencePackRow(row) &&
      !isKurashikiStayPackRow(row) &&
      !isSightsCategory(row.category)
    ) {
      continue;
    }
  ...
  const ranked = rankKurashikiSeeRows(pack);
  const onsen = pack.filter(isKurashikiOnsenPackRow);
  const experience = pack.filter(isKurashikiExperiencePackRow);
  const stay = pack.filter(isKurashikiStayPackRow);
  for (const row of [...stay, ...onsen, ...experience, ...ranked]) {
    const kind: ListingKind = isKurashikiOnsenPackRow(row)
      ? 'onsen'
      : isKurashikiExperiencePackRow(row)
        ? 'experience'
        : isKurashikiStayPackRow(row)
          ? 'stay'
          : 'sights';
    out.push(fromPack(row, 'kurashiki', kind, kurashikiSightPhoto(row.name_ja)));
  }
  return out;
}
...
function tsuyamaListings(): PublicListing[] {
  const town = lookupTown('tsuyama')!;
  const out: PublicListing[] = town.travelAll.map((row) =>
    fromTravel(row, 'tsuyama', tsuyamaSightPhoto(row.name_ja))
  );
  const seen = new Set<string>();
  const pack: FacilityRow[] = [];
  for (const row of town.rows) {
    if (TSUYAMA_DINING_NAME_SET.has(row.name_ja)) continue;
    if (
      !isTsuyamaOnsenPackRow(row) &&
      !isTsuyamaExperiencePackRow(row) &&
      !isTsuyamaStayPackRow(row) &&
      !isSightsCategory(row.category)
    ) {
      continue;
    }
  ...
  const ranked = rankTsuyamaSeeRows(pack);
  const onsen = pack.filter(isTsuyamaOnsenPackRow);
  const experience = pack.filter(isTsuyamaExperiencePackRow);
  const stay = pack.filter(isTsuyamaStayPackRow);
  for (const row of [...stay, ...onsen, ...experience, ...ranked]) {
    const kind: ListingKind = isTsuyamaOnsenPackRow(row)
      ? 'onsen'
      : isTsuyamaExperiencePackRow(row)
        ? 'experience'
        : isTsuyamaStayPackRow(row)
          ? 'stay'
          : 'sights';
    out.push(fromPack(row, 'tsuyama', kind, tsuyamaSightPhoto(row.name_ja)));
  }
  return out;
}
...
function jinseikogenListings(): PublicListing[] {
  ...
  const out: PublicListing[] = town.travelAll.map((row) =>
    fromTravel(row, 'jinseikogen', jinseikogenSightPhoto(row.name_ja))
  );
  const seen = new Set<string>();
  const pack: FacilityRow[] = [];
  for (const row of town.rows) {
    if (JINSEIKOGEN_DINING_NAME_SET.has(row.name_ja)) continue;
    if (
      !isJinseikogenOnsenPackRow(row) &&
      !isJinseikogenExperiencePackRow(row) &&
      !isJinseikogenStayPackRow(row) &&
      !isSightsCategory(row.category)
    ) {
      continue;
    }
  ...
  const ranked = rankJinseikogenSeeRows(pack);
  const onsen = pack.filter(isJinseikogenOnsenPackRow);
  const experience = pack.filter(isJinseikogenExperiencePackRow);
  const stay = pack.filter(isJinseikogenStayPackRow);
  for (const row of [...stay, ...onsen, ...experience, ...ranked]) {
    const kind: ListingKind = isJinseikogenOnsenPackRow(row)
      ? 'onsen'
      : isJinseikogenExperiencePackRow(row)
        ? 'experience'
        : isJinseikogenStayPackRow(row)
          ? 'stay'
          : 'sights';
    out.push(fromPack(row, 'jinseikogen', kind, jinseikogenSightPhoto(row.name_ja)));
  }
  return out;
}

function seraListings(): PublicListing[] {
  const town = lookupTown('sera')!;
  const out: PublicListing[] = town.travelAll.map((row) =>
    fromTravel(row, 'sera', seraSightPhoto(row.name_ja))
  );
  const seen = new Set<string>();
  const pack: FacilityRow[] = [];
  for (const row of town.rows) {
    if (SERA_DINING_NAME_SET.has(row.name_ja)) continue;
    if (
      !isSeraOnsenPackRow(row) &&
      !isSeraExperiencePackRow(row) &&
      !isSeraStayPackRow(row) &&
      !isSightsCategory(row.category)
    ) {
      continue;
    }
  ...
  const ranked = rankSeraSeeRows(pack);
  const onsen = pack.filter(isSeraOnsenPackRow);
  const experience = pack.filter(isSeraExperiencePackRow);
  const stay = pack.filter(isSeraStayPackRow);
  for (const row of [...stay, ...onsen, ...experience, ...ranked]) {
    const kind: ListingKind = isSeraOnsenPackRow(row)
      ? 'onsen'
      : isSeraExperiencePackRow(row)
        ? 'experience'
        : isSeraStayPackRow(row)
          ? 'stay'
          : 'sights';
    out.push(fromPack(row, 'sera', kind, seraSightPhoto(row.name_ja)));
  }
  return out;
}

function osakikamijimaListings(): PublicListing[] {
  const town = lookupTown('osakikamijima')!;
  const out: PublicListing[] = town.travelAll.map((row) =>
    fromTravel(row, 'osakikamijima', osakikamijimaSightPhoto(row.name_ja))
  );
  const seen = new Set<string>();
  const pack: FacilityRow[] = [];
  for (const row of town.rows) {
    if (OSAKIKAMIJIMA_DINING_NAME_SET.has(row.name_ja)) continue;
    if (
      !isOsakikamijimaOnsenPackRow(row) &&
      !isOsakikamijimaExperiencePackRow(row) &&
      !isOsakikamijimaStayPackRow(row) &&
      !isSightsCategory(row.category)
    ) {
      continue;
    }
  ...
  const ranked = rankOsakikamijimaSeeRows(pack);
  const onsen = pack.filter(isOsakikamijimaOnsenPackRow);
  const experience = pack.filter(isOsakikamijimaExperiencePackRow);
  const stay = pack.filter(isOsakikamijimaStayPackRow);
  for (const row of [...stay, ...onsen, ...experience, ...ranked]) {
    const kind: ListingKind = isOsakikamijimaOnsenPackRow(row)
      ? 'onsen'
      : isOsakikamijimaExperiencePackRow(row)
        ? 'experience'
        : isOsakikamijimaStayPackRow(row)
          ? 'stay'
          : 'sights';
    out.push(fromPack(row, 'osakikamijima', kind, osakikamijimaSightPhoto(row.name_ja)));
  }
  return out;
}

function kitahiroshimaListings(): PublicListing[] {
  const town = lookupTown('kitahiroshima')!;
  const out: PublicListing[] = town.travelAll.map((row) =>
    fromTravel(row, 'kitahiroshima', kitahiroshimaSightPhoto(row.name_ja))
  );
  const seen = new Set<string>();
  const pack: FacilityRow[] = [];
  for (const row of town.rows) {
    if (KITAHIROSHIMA_DINING_NAME_SET.has(row.name_ja)) continue;
    if (
      !isKitahiroshimaOnsenPackRow(row) &&
      !isKitahiroshimaExperiencePackRow(row) &&
      !isKitahiroshimaStayPackRow(row) &&
      !isSightsCategory(row.category)
    ) {
      continue;
    }
  ...
  const ranked = rankKitahiroshimaSeeRows(pack);
  const onsen = pack.filter(isKitahiroshimaOnsenPackRow);
  const experience = pack.filter(isKitahiroshimaExperiencePackRow);
  const stay = pack.filter(isKitahiroshimaStayPackRow);
  for (const row of [...stay, ...onsen, ...experience, ...ranked]) {
    const kind: ListingKind = isKitahiroshimaOnsenPackRow(row)
      ? 'onsen'
      : isKitahiroshimaExperiencePackRow(row)
        ? 'experience'
        : isKitahiroshimaStayPackRow(row)
          ? 'stay'
          : 'sights';
    out.push(fromPack(row, 'kitahiroshima', kind, kitahiroshimaSightPhoto(row.name_ja)));
  }
  return out;
}
...
function kumanoListings(): PublicListing[] {
  ...
  const out: PublicListing[] = town.travelAll.map((row) =>
    fromTravel(row, 'kumano', kumanoSightPhoto(row.name_ja))
  );
  const seen = new Set<string>();
  const pack: FacilityRow[] = [];
  for (const row of town.rows) {
    if (KUMANO_DINING_NAME_SET.has(row.name_ja)) continue;
    if (
      !isKumanoOnsenPackRow(row) &&
      !isKumanoExperiencePackRow(row) &&
      !isKumanoStayPackRow(row) &&
      !isSightsCategory(row.category)
    ) {
      continue;
    }
  ...
  const ranked = rankKumanoSeeRows(pack);
  const onsen = pack.filter(isKumanoOnsenPackRow);
  const experience = pack.filter(isKumanoExperiencePackRow);
  const stay = pack.filter(isKumanoStayPackRow);
  for (const row of [...stay, ...onsen, ...experience, ...ranked]) {
    const kind: ListingKind = isKumanoOnsenPackRow(row)
      ? 'onsen'
      : isKumanoExperiencePackRow(row)
        ? 'experience'
        : isKumanoStayPackRow(row)
          ? 'stay'
          : 'sights';
    out.push(fromPack(row, 'kumano', kind, kumanoSightPhoto(row.name_ja)));
  }
  return out;
}

function etajimaListings(): PublicListing[] {
  const town = lookupTown('etajima')!;
  const out: PublicListing[] = town.travelAll.map((row) =>
    fromTravel(row, 'etajima', etajimaSightPhoto(row.name_ja))
  );
  const seen = new Set<string>();
  const pack: FacilityRow[] = [];
  for (const row of town.rows) {
    if (ETAJIMA_DINING_NAME_SET.has(row.name_ja)) continue;
    if (
      !isEtajimaOnsenPackRow(row) &&
      !isEtajimaExperiencePackRow(row) &&
      !isEtajimaStayPackRow(row) &&
      !isSightsCategory(row.category)
    ) {
      continue;
    }
  ...
  const ranked = rankEtajimaSeeRows(pack);
  const onsen = pack.filter(isEtajimaOnsenPackRow);
  const experience = pack.filter(isEtajimaExperiencePackRow);
  const stay = pack.filter(isEtajimaStayPackRow);
  for (const row of [...stay, ...onsen, ...experience, ...ranked]) {
    const kind: ListingKind = isEtajimaOnsenPackRow(row)
      ? 'onsen'
      : isEtajimaExperiencePackRow(row)
        ? 'experience'
        : isEtajimaStayPackRow(row)
          ? 'stay'
          : 'sights';
    out.push(fromPack(row, 'etajima', kind, etajimaSightPhoto(row.name_ja)));
  }
  return out;
}

function akitakataListings(): PublicListing[] {
  const town = lookupTown('akitakata')!;
  const out: PublicListing[] = town.travelAll.map((row) =>
    fromTravel(row, 'akitakata', akitakataSightPhoto(row.name_ja))
  );
  const seen = new Set<string>();
  const pack: FacilityRow[] = [];
  for (const row of town.rows) {
    if (AKITAKATA_DINING_NAME_SET.has(row.name_ja)) continue;
    if (
      !isAkitakataOnsenPackRow(row) &&
      !isAkitakataExperiencePackRow(row) &&
      !isAkitakataStayPackRow(row) &&
      !isSightsCategory(row.category)
    ) {
      continue;
    }
  ...
  const ranked = rankAkitakataSeeRows(pack);
  const onsen = pack.filter(isAkitakataOnsenPackRow);
  const experience = pack.filter(isAkitakataExperiencePackRow);
  const stay = pack.filter(isAkitakataStayPackRow);
  for (const row of [...stay, ...onsen, ...experience, ...ranked]) {
    const kind: ListingKind = isAkitakataOnsenPackRow(row)
      ? 'onsen'
      : isAkitakataExperiencePackRow(row)
        ? 'experience'
        : isAkitakataStayPackRow(row)
          ? 'stay'
          : 'sights';
    out.push(fromPack(row, 'akitakata', kind, akitakataSightPhoto(row.name_ja)));
  }
  return out;
}

function hatsukaichiListings(): PublicListing[] {
  const town = lookupTown('hatsukaichi')!;
  const out: PublicListing[] = town.travelAll.map((row) =>
    fromTravel(row, 'hatsukaichi', hatsukaichiSightPhoto(row.name_ja))
  );
  const seen = new Set<string>();
  const pack: FacilityRow[] = [];
  for (const row of town.rows) {
    if (HATSUKAICHI_DINING_NAME_SET.has(row.name_ja)) continue;
    if (
      !isHatsukaichiOnsenPackRow(row) &&
      !isHatsukaichiExperiencePackRow(row) &&
      !isHatsukaichiStayPackRow(row) &&
      !isSightsCategory(row.category)
    ) {
      continue;
    }
  ...
  const ranked = rankHatsukaichiSeeRows(pack);
  const onsen = pack.filter(isHatsukaichiOnsenPackRow);
  const experience = pack.filter(isHatsukaichiExperiencePackRow);
  const stay = pack.filter(isHatsukaichiStayPackRow);
  for (const row of [...stay, ...onsen, ...experience, ...ranked]) {
    const kind: ListingKind = isHatsukaichiOnsenPackRow(row)
      ? 'onsen'
      : isHatsukaichiExperiencePackRow(row)
        ? 'experience'
        : isHatsukaichiStayPackRow(row)
          ? 'stay'
          : 'sights';
    out.push(fromPack(row, 'hatsukaichi', kind, hatsukaichiSightPhoto(row.name_ja)));
  }
  return out;
}

function higashihiroshimaListings(): PublicListing[] {
  const town = lookupTown('higashihiroshima')!;
  const out: PublicListing[] = town.travelAll.map((row) =>
    fromTravel(row, 'higashihiroshima', higashihiroshimaSightPhoto(row.name_ja))
  );
  const seen = new Set<string>();
  const pack: FacilityRow[] = [];
  for (const row of town.rows) {
    if (HIGASHIHIROSHIMA_DINING_NAME_SET.has(row.name_ja)) continue;
    if (
      !isHigashihiroshimaOnsenPackRow(row) &&
      !isHigashihiroshimaExperiencePackRow(row) &&
      !isHigashihiroshimaStayPackRow(row) &&
      !isSightsCategory(row.category)
    ) {
      continue;
    }
  ...
  const ranked = rankHigashihiroshimaSeeRows(pack);
  const onsen = pack.filter(isHigashihiroshimaOnsenPackRow);
  const experience = pack.filter(isHigashihiroshimaExperiencePackRow);
  const stay = pack.filter(isHigashihiroshimaStayPackRow);
  for (const row of [...stay, ...onsen, ...experience, ...ranked]) {
    const kind: ListingKind = isHigashihiroshimaOnsenPackRow(row)
      ? 'onsen'
      : isHigashihiroshimaExperiencePackRow(row)
        ? 'experience'
        : isHigashihiroshimaStayPackRow(row)
          ? 'stay'
          : 'sights';
    out.push(fromPack(row, 'higashihiroshima', kind, higashihiroshimaSightPhoto(row.name_ja)));
  }
  return out;
}

function otakeListings(): PublicListing[] {
  const town = lookupTown('otake')!;
  const out: PublicListing[] = town.travelAll.map((row) =>
    fromTravel(row, 'otake', otakeSightPhoto(row.name_ja))
  );
  const seen = new Set<string>();
  const pack: FacilityRow[] = [];
  for (const row of town.rows) {
    if (OTAKE_DINING_NAME_SET.has(row.name_ja)) continue;
    if (
      !isOtakeOnsenPackRow(row) &&
      !isOtakeExperiencePackRow(row) &&
      !isOtakeStayPackRow(row) &&
      !isSightsCategory(row.category)
    ) {
      continue;
    }
  ...
  const ranked = rankOtakeSeeRows(pack);
  const onsen = pack.filter(isOtakeOnsenPackRow);
  const experience = pack.filter(isOtakeExperiencePackRow);
  const stay = pack.filter(isOtakeStayPackRow);
  for (const row of [...stay, ...onsen, ...experience, ...ranked]) {
    const kind: ListingKind = isOtakeOnsenPackRow(row)
      ? 'onsen'
      : isOtakeExperiencePackRow(row)
        ? 'experience'
        : isOtakeStayPackRow(row)
          ? 'stay'
          : 'sights';
    out.push(fromPack(row, 'otake', kind, otakeSightPhoto(row.name_ja)));
  }
  return out;
}

function shobaraListings(): PublicListing[] {
  const town = lookupTown('shobara')!;
  const out: PublicListing[] = town.travelAll.map((row) =>
    fromTravel(row, 'shobara', shobaraSightPhoto(row.name_ja))
  );
  const seen = new Set<string>();
  const pack: FacilityRow[] = [];
  for (const row of town.rows) {
    if (SHOBARA_DINING_NAME_SET.has(row.name_ja)) continue;
    if (
      !isShobaraOnsenPackRow(row) &&
      !isShobaraExperiencePackRow(row) &&
      !isShobaraStayPackRow(row) &&
      !isSightsCategory(row.category)
    ) {
      continue;
    }
  ...
  const ranked = rankShobaraSeeRows(pack);
  const onsen = pack.filter(isShobaraOnsenPackRow);
  const experience = pack.filter(isShobaraExperiencePackRow);
  const stay = pack.filter(isShobaraStayPackRow);
  for (const row of [...stay, ...onsen, ...experience, ...ranked]) {
    const kind: ListingKind = isShobaraOnsenPackRow(row)
      ? 'onsen'
      : isShobaraExperiencePackRow(row)
        ? 'experience'
        : isShobaraStayPackRow(row)
          ? 'stay'
          : 'sights';
    out.push(fromPack(row, 'shobara', kind, shobaraSightPhoto(row.name_ja)));
  }
  return out;
}

function fuchuListings(): PublicListing[] {
  const town = lookupTown('fuchu')!;
  const out: PublicListing[] = town.travelAll.map((row) =>
    fromTravel(row, 'fuchu', fuchuSightPhoto(row.name_ja))
  );
  const seen = new Set<string>();
  const pack: FacilityRow[] = [];
  for (const row of town.rows) {
    if (FUCHU_DINING_NAME_SET.has(row.name_ja)) continue;
    if (
      !isFuchuOnsenPackRow(row) &&
      !isFuchuExperiencePackRow(row) &&
      !isFuchuStayPackRow(row) &&
      !isSightsCategory(row.category)
    ) {
      continue;
    }
  ...
  const ranked = rankFuchuSeeRows(pack);
  const onsen = pack.filter(isFuchuOnsenPackRow);
  const experience = pack.filter(isFuchuExperiencePackRow);
  const stay = pack.filter(isFuchuStayPackRow);
  for (const row of [...stay, ...onsen, ...experience, ...ranked]) {
    const kind: ListingKind = isFuchuOnsenPackRow(row)
      ? 'onsen'
      : isFuchuExperiencePackRow(row)
        ? 'experience'
        : isFuchuStayPackRow(row)
          ? 'stay'
          : 'sights';
    out.push(fromPack(row, 'fuchu', kind, fuchuSightPhoto(row.name_ja)));
  }
  return out;
}

function fukuyamaListings(): PublicListing[] {
  const town = lookupTown('fukuyama')!;
  const out: PublicListing[] = town.travelAll.map((row) =>
    fromTravel(row, 'fukuyama', fukuyamaSightPhoto(row.name_ja))
  );
  const seen = new Set<string>();
  const pack: FacilityRow[] = [];
  for (const row of town.rows) {
    if (FUKUYAMA_DINING_NAME_SET.has(row.name_ja)) continue;
    if (
      !isFukuyamaOnsenPackRow(row) &&
      !isFukuyamaExperiencePackRow(row) &&
      !isFukuyamaStayPackRow(row) &&
      !isSightsCategory(row.category)
    ) {
      continue;
    }
  ...
  const ranked = rankFukuyamaSeeRows(pack);
  const onsen = pack.filter(isFukuyamaOnsenPackRow);
  const experience = pack.filter(isFukuyamaExperiencePackRow);
  const stay = pack.filter(isFukuyamaStayPackRow);
  for (const row of [...stay, ...onsen, ...experience, ...ranked]) {
    const kind: ListingKind = isFukuyamaOnsenPackRow(row)
      ? 'onsen'
      : isFukuyamaExperiencePackRow(row)
        ? 'experience'
        : isFukuyamaStayPackRow(row)
          ? 'stay'
          : 'sights';
    out.push(fromPack(row, 'fukuyama', kind, fukuyamaSightPhoto(row.name_ja)));
  }
  return out;
}

function onomichiListings(): PublicListing[] {
  const town = lookupTown('onomichi')!;
  const out: PublicListing[] = town.travelAll.map((row) =>
    fromTravel(row, 'onomichi', onomichiSightPhoto(row.name_ja))
  );
  const seen = new Set<string>();
  const pack: FacilityRow[] = [];
  for (const row of town.rows) {
    if (ONOMICHI_DINING_NAME_SET.has(row.name_ja)) continue;
    if (
      !isOnomichiOnsenPackRow(row) &&
      !isOnomichiExperiencePackRow(row) &&
      !isOnomichiStayPackRow(row) &&
      !isSightsCategory(row.category)
    ) {
      continue;
    }
  ...
  const ranked = rankOnomichiSeeRows(pack);
  const onsen = pack.filter(isOnomichiOnsenPackRow);
  const experience = pack.filter(isOnomichiExperiencePackRow);
  const stay = pack.filter(isOnomichiStayPackRow);
  for (const row of [...stay, ...onsen, ...experience, ...ranked]) {
    const kind: ListingKind = isOnomichiOnsenPackRow(row)
      ? 'onsen'
      : isOnomichiExperiencePackRow(row)
        ? 'experience'
        : isOnomichiStayPackRow(row)
          ? 'stay'
          : 'sights';
    out.push(fromPack(row, 'onomichi', kind, onomichiSightPhoto(row.name_ja)));
  }
  return out;
}
...
const CACHE: Record<ReadySlug, PublicListing[]> = {
  ...
  onomichi: onomichiListings(),
  fukuyama: fukuyamaListings(),
  fuchu: fuchuListings(),
  miyoshishi: miyoshishiListings(),
  shobara: shobaraListings(),
  otake: otakeListings(),
  higashihiroshima: higashihiroshimaListings(),
  hatsukaichi: hatsukaichiListings(),
  akitakata: akitakataListings(),
  ...
  kurashiki: kurashikiListings(),
  tsuyama: tsuyamaListings(),
  sera: seraListings()
};
